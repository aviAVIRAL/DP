...
def f(i, j, m, matrix, dp):
    ...
    up = matrix[i][j] + f(i - 1, j, m, matrix, dp)
    leftDiagonal = matrix[i][j] + f(i - 1, j - 1, m, matrix, dp)
    rightDiagonal = matrix[i][j] + f(i - 1, j + 1, m, matrix, dp)
