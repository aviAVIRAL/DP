...
def f(i, j1, j2, n, m, grid, dp):
    # Base cases:
    # - If either of the indj1ces is out of bounds, return a large negative value
...
def maximumChocolates(n, m, grid):
    ...
    dp = [[[-1 for j in range(max(n,m))] for i in range(max(n,m))] for k in range(max(n,m))]
...
def main():
    # Define the input matrix and its dj1mensions
